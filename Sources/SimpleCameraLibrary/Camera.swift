...
struct Ingredient: Codable {
    ...
    let is_in_taxonomy: Int?
    let vegan: String?
    let vegetarian: String?
    let ciqual_food_code: String?
    let ecobalyse_code: String?
    let quantity: String?
...
struct OpenFoodFactsProduct: Codable {
    ...
    let ingredients: [Ingredient]? // Add this field to capture ingredients
...
public class ProductScannerService: NSObject, ObservableObject {
    ...
    private func extractVolumeFromName(_ productName: String?) -> String? {
        ...
        if let match = volumeRegex?.firstMatch(in: name, options: [], range: range),
           let matchRange = Range(match.range(at: 0), in: name) {
        ...
        URLSession.shared.dataTask(with: url) { [weak self] data, response, error in
            ...
            DispatchQueue.main.async {
                ...
                do {
                    ...
                    // Extract water percentage from ingredients
                    var waterPercentage: Double?
                    if let ingredients = product.ingredients {
                        waterPercentage = self.extractWaterPercentage(from: ingredients)
                    }
    ...
    private func extractWaterPercentage(from ingredients: [Ingredient]) -> Double? {
        for ingredient in ingredients {
            if let id = ingredient.id,
               id.contains("water"), // Look for ingredients containing "water"
               let percentEstimate = ingredient.percent_estimate {
